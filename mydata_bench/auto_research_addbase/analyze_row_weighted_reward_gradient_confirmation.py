...
def main():
    root = DEST/'row_weighted_reward_gradient_confirmation_v1'
    ...
    if claim.get('family')!='F9_sample_mean_ordinal_signed_all_query_reward_gradient' or claim.get('policy_sha256')!=plan['policy_sha256']:
        raise ValueError('Reserved claim does not belong to the frozen F9 method and policy')
    ...
    report += ['', '统计支持输入数：'+json.dumps(supported_counts, ensure_ascii=False)+'。科学证据门槛：'+str(evidence_pass)+'。', '',
        ...
        'F9用独立公开训练标签选头，骨干冻结，属于监督辅助学习。全部原生评分档位保留，无ROI回退baseline。', '',
