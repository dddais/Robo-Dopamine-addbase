"""Freeze learned head hashes and old-development inference after all F6 fits."""
import json
import pathlib
import yaml
...
def main():
    ...
    for item in plan['configurations']:
        ...
        source=DEST/'reward_gradient_v1'/name/'fit'
        ...
        conditions.extend([
            ...
            dict(name='raw_readout_k48',k=48,kind='bias',bias=1.,scope='all_frames',query_scope='readout',ranking='original'),
            dict(name='learned_positive_k48',k=48,kind='reward_gradient',bias=1.,scope='all_frames',signed=False)])
        cfg.update(clean_source_dir=cfg['output_dir'],output_dir=str(DEST/f'reward_gradient_development_v1/{name}'),
            ...
            primary_k=48,k_neighborhood=[32,48,64],stage='F6_externally_fitted_heads_old234_development',
            output_score_adjustment='none',ranking_supervised_source='separate official train fitting partition only')
        for filename in ['reward_gradient_attention.py','reward_gradient_runtime.py','reward_gradient_development.py','freeze_reward_gradient_development.py']:
            path=ROOT/'mydata_bench/auto_research_addbase'/filename
            cfg['source_sha256'][str(path.relative_to(ROOT))]=digest(path)
        path=ROOT/f'mydata_bench/configs/v2_crossmodel/addbase_robust_reward_gradient_development_v1_{name}.yaml'
    ...
    create_json(DEST/'reward_gradient_development_frozen_plan_v1.json',dict(time=timestamp(),configurations=configs,
        ...
        independent_training_validation_still_required=True))
