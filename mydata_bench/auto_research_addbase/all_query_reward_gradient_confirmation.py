"""Once-only reserved2831 F7 inference; no labels or parameter selection."""
import argparse
import fcntl
import json
import os
...
def main():
    ...
    p.add_argument('--stage', choices=['engineering', 'confirmation'], required=True)
    args = p.parse_args()
    ...
    if len(ids) != len(set(ids)) or {s['example_id'] for s in samples} != set(ids) or len(samples) != len(ids):
        raise ValueError('Unknown/duplicate frozen reserved-test IDs')
    if any(s['preparation_status'] != 'ready' or 'reward' in s or 'label' in s for s in samples):
        raise ValueError('Reserved input preparation/label boundary violation')
    if args.stage == 'confirmation' and len(ids) != 2831:
        raise ValueError('Reserved confirmation requires the complete2831 denominator')
    if digest(cfg['confirmation_claim_file']) != cfg['confirmation_claim_sha256']:
        raise ValueError('Once-only confirmation claim changed')
    ...
    if args.stage == 'confirmation':
        if not json.loads((out.parent/'engineering/engineering_audit_v1.json').read_text())['passed']:
            raise ValueError('Actual reserved-test engineering gate failed')
    ...
    create_json(out/'launch_v1.json', dict(time=timestamp(), pid=os.getpid(), gpu=os.environ.get('CUDA_VISIBLE_DEVICES'),
        config_sha256=digest(args.config), stage=args.stage, labels_read=False, reserved_test_inference=True, test_performance_not_joined=True))
    ...
    if args.stage == 'engineering':
        names = ['baseline', 'native_generation_baseline', 'original_all_k48', 'reward_gradient_k48', 'reward_gradient_zero_k48', 'learned_readout_k48']
            ...
            audits.append(dict(example_id=sample['example_id'], grounding_fallback=fallback,
                checks=dict(all_conditions_ok=all(r['status'] == 'ok' for r in current.values()),
                    ...
                    target_hook_or_fallback=(target.get('baseline_fallback', False) and
                        ...
                        if fallback else (target.get('hook_diagnostics', {}).get('calls', 0) > 0 and
                            target.get('hook_diagnostics',{}).get('all_causal_queries',False) and
                            current['learned_readout_k48'].get('hook_diagnostics',{}).get('only_readout_query_changed',False)))))
    ...
    if args.stage == 'engineering':
        ...
        if not passed:
            raise ValueError('Reserved-test engineering failed; records retained')
