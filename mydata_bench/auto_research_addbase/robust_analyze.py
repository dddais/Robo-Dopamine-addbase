"""Offline F1 analysis. Never read external confirmation labels here."""
import argparse
import collections
import json
import pathlib
...
def main():
    ...
    parser.add_argument('--family', choices=['f1', 'f2', 'f3', 'f4', 'f5', 'f6'], default='f1')
    ...
    if args.references_only:
        ...
        return
    cases = {}
    experiment_root = DEST / {'f1': 'frame_transport_v1', 'f2': 'frame_transport_vcd_v1', 'f3': 'specificity_v1', 'f4': 'joint_task_v1', 'f5': 'task_interaction_v1', 'f6': 'reward_gradient_development_v1'}[args.family]
    target_prefix = {'f1': 'frame_transport', 'f2': 'frame_transport', 'f3': 'specificity', 'f4': 'joint_task', 'f5': 'task_interaction', 'f6': 'reward_gradient'}[args.family]
    for folder in sorted(experiment_root.iterdir()):
        ...
        for k in cfg['k_neighborhood']:
            ...
            if args.family == 'f6' and k == 48:
                comparisons['raw_readout'] = summaries['raw_readout_k48']
