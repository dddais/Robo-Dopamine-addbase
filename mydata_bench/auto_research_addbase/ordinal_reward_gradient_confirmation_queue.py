...
from .f8_confirmation_priority import confirmation_priority
...
def worker(gpu, configurations):
    ...
    events = DEST/f'f8_confirmation_gpu{gpu}_events_v1.jsonl'
    failures = []
    for cfg in configurations:
        ...
        for stage in ['engineering', 'confirmation']:
            ...
            with (DEST/f'f8_confirmation_{name}_{stage}_stdout_v1.log').open('x') as handle:
            ...
            if code:
                ...
                break
    create_json(DEST/f'f8_confirmation_gpu{gpu}_completion_v1.json', dict(time=timestamp(), failures=failures,
        cases=[r['model']+'_'+r['protocol'] for r in configurations]))
    return failures


def main():
    policy_path = DEST/'f8_confirmation_policy_v1.json'
    policy = json.loads(policy_path.read_text())
    create_json(DEST/'f8_confirmation_queue_plan_v1.json', dict(time=timestamp(), pid=os.getpid(),
    ...
    launch = json.loads((DEST/'f8_full_old_queue_launch_v1.json').read_text())
    completion = DEST/'f8_full_old_evaluation_and_analysis_completion_v1.json'
    while not completion.exists():
        process = pathlib.Path('/proc')/str(launch['pid'])/'cmdline'
        if not process.exists() or b'mydata_bench.auto_research_addbase.ordinal_reward_gradient_full_old_queue' not in process.read_bytes():
            eligibility = DEST/'f8_full_old_queue_eligibility_v1.json'
            no_eligibility = eligibility.exists() and not json.loads(eligibility.read_text())['eligible']
            create_json(DEST/'f8_confirmation_queue_eligibility_v1.json', dict(time=timestamp(), eligible=False,
                reason='F8 failed joint old234/validation eligibility' if no_eligibility else 'Full846 prerequisite stopped without successful analysis',
    ...
    analysis_path = DEST/'ordinal_reward_gradient_full_old_v1/analysis_v1.json'
    ...
    while True:
        ...
        if priority['state']=='waiting':
            time.sleep(20)
            continue
        create_json(DEST/'f8_confirmation_queue_priority_ineligible_v1.json',dict(time=timestamp(),
            eligible=False,priority=priority,reserved_test_reward_inference_started=False,final_goal_completed=False))
        return
    create_json(DEST/'f8_confirmation_queue_eligibility_v1.json', dict(time=timestamp(), eligible=True,
        cases=names, full846_analysis_sha256=digest(analysis_path)))
    with (DEST/'f8_confirmation_freeze_stdout_v1.log').open('x') as handle:
        result = subprocess.run([sys.executable, '-u', '-m', 'mydata_bench.auto_research_addbase.freeze_ordinal_reward_gradient_confirmation'],
            cwd=ROOT, stdout=handle, stderr=subprocess.STDOUT)
    if result.returncode:
        raise RuntimeError('Reserved-test freeze failed; inspect without changing selected method')
    plan = json.loads((DEST/'ordinal_reward_gradient_confirmation_v1/frozen_plan_v1.json').read_text())
    ...
    create_json(DEST/'f8_confirmation_inference_completion_v1.json', dict(time=timestamp(), failures=failures,
    ...
    with (DEST/'f8_confirmation_analysis_stdout_v1.log').open('x') as handle:
        result = subprocess.run([sys.executable, '-u', '-m', 'mydata_bench.auto_research_addbase.analyze_ordinal_reward_gradient_confirmation'],
    ...
    create_json(DEST/'f8_confirmation_evaluation_and_analysis_completion_v1.json', dict(time=timestamp(),
