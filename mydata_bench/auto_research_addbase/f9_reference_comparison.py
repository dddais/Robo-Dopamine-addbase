"""Load prospectively fixed F8 episode-weighted ordinal comparisons without changing F9 inference."""
import json
import pathlib
import yaml
...
def f8_primary_rows(policy, case, stage, current_config, ids):
    item = policy['fixed_F8_comparator_configs'][stage][case]
    if digest(item['path']) != item['sha256']:
        raise ValueError('Prospectively frozen F8 comparator configuration changed')
    ...
    for field in ['model', 'protocol', 'model_path', 'processor_path', 'input_sha256', 'example_ids_file']:
        if cfg.get(field) != current_config.get(field):
            raise ValueError('F8/F9 comparison mismatch: '+field)
    ...
    if digest(predictions) != done['predictions_sha256'] or json.loads((out/'config.json').read_text()) != cfg:
        raise ValueError('F8 completed comparator provenance mismatch')
    ...
    if len(keys) != len(set(keys)) or set(keys) != expected:
        raise ValueError('F8 comparator is incomplete or duplicated')
