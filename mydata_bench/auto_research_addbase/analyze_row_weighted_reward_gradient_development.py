"""F9 complete old234 analysis: original scope, both thresholds, fixed F7 comparison."""
import collections
import json
import pathlib
import yaml
...
def main():
    ...
    for path, expected in policy['evaluation_source_sha256'].items():
        if digest(ROOT/path) != expected:
            raise ValueError('Frozen F9 analysis source changed: '+path)
    plan_path = DEST/'row_weighted_reward_gradient_development_frozen_plan_v1.json'
    ...
    for item in plan['configurations']:
        if digest(item['path']) != item['sha256']:
            raise ValueError('Frozen F9 development configuration changed')
        cfg = yaml.safe_load(pathlib.Path(item['path']).read_text())
        if cfg['evaluation_policy_sha256'] != digest(policy_path):
            raise ValueError('Wrong F9 evaluation policy')
        ...
        if digest(path) != done['predictions_sha256'] or json.loads((out/'config.json').read_text()) != cfg:
            raise ValueError('F9 prediction/config provenance mismatch')
        ...
        if len(keys) != len(set(keys)) or set(keys) != expected or len(keys) != done['expected_records']:
            raise ValueError('Incomplete or duplicated F9 old234 predictions')
        loaded.append((item['model']+'_'+item['protocol'], cfg, done, rows))
    fixed_references = {name: (
        f7_primary_rows(policy, name, 'development', cfg, ids),
        f8_primary_rows(policy, name, 'development', cfg, ids)) for name, cfg, _, _ in loaded}
    ...
    for name, cfg, done, rows in loaded:
        ...
        f7 = summarize(fixed_references[name][0], labels, ids)
        f8 = summarize(fixed_references[name][1], labels, ids)
        effects = {}
        for k in [32, 48, 64]:
            ...
            if k == 48:
                comparisons.update(raw_all=summaries['raw_all_k48'], same_heads_readout_only=summaries['learned_readout_k48'],
                    ...
                    F8_episode_weighted_ordinal_heads=f8)
    ...
    path = DEST/('f9_frame_transport_analysis_'+timestamp().replace(':', '').replace('+', '_')+'.json')
