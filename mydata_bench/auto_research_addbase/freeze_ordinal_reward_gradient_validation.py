"""Freeze validation configs after all fits, before any F8 evaluation scores."""
import json
import pathlib
import yaml
...
def main():
    ...
    if queue_done['failures']:
        raise ValueError('At least one required F8 fitting case failed')
    ...
    if len(dev_plan['configurations']) != 15:
        raise ValueError('Require all fifteen F8 configurations')
    ...
    engineering_path = OUT/'f8_validation_engineering_ids_v1.json'
    create_json(engineering_path, grounded[:2]+fallback[:1])
    tracks_path = OUT/'f8_validation_tracking_sha256_v1.json'
    ...
    for item in dev_plan['configurations']:
        if digest(item['path']) != item['sha256']:
            raise ValueError('F8 development config changed')
        ...
        for stage in ['engineering', 'development']:
            if (pathlib.Path(cfg['output_dir'])/stage/'launch_v1.json').exists():
                raise ValueError('F8 evaluation already started before validation freeze')
        ...
        cfg.update(output_dir=str(DEST/f'ordinal_reward_gradient_validation_v1/{name}'),
            ...
            stage='F8_public_train_model_selection_reused_after_F6', inference_reads_labels=False,
        ...
        path = ROOT/f'mydata_bench/configs/v2_crossmodel/addbase_robust_ordinal_reward_gradient_validation_v1_{name}.yaml'
    ...
    create_json(DEST/'ordinal_reward_gradient_validation_frozen_plan_v1.json', dict(time=timestamp(), configurations=configs,
        ...
        fit_examples=len(fit), expected_cases=15, this_candidate_validation_performance_not_observed_before_freeze=True,validation_reused_for_model_selection=True,
        analyze_labels_only_after_all_cases_finish=True, reserved_test_performance_not_opened=True))
    print('Frozen F8 training model-selection validation:', len(samples), 'rows,', len(configs), 'cases', flush=True)
