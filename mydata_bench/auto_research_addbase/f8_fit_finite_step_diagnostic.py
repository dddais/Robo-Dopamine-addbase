"""One frozen Qwen training-only b1/k48 F8 versus F7 comparison, no parameter search."""
import argparse
import collections
import fcntl
import json
    ...
    for branch in ['baseline','F7','F8']:
        ...
        result[branch]=dict(row_mean={k:float(np.mean([r['metrics'][branch][k] for r in records])) for k in keys},
            ...
            by_grade={str(g):dict(n=sum(r['grade']==g for r in records),
                **{k:float(np.mean([r['metrics'][branch][k] for r in records if r['grade']==g])) for k in keys}) for g in [1,2,3,4,5]})
...
def main():
    ...
    for field in ['training_input','fit_ids','fit_labels','ranking','f7_reference_records','f7_ranking','f8_fit_records']:
    ...
    cached_rows=list(read_rows(cfg['f7_reference_records_file']));cache={r['example_id']:r for r in cached_rows}
    ...
    slopes={branch:sum(gradients[(r['layer'],r['head'])]*r['signed_bias_direction'] for r in order[:48])
            for branch,order in [('F7',f7_ranking),('F8',ranking)]}
    ...
    for index,sample in enumerate(samples,1):
        ...
        try:
            ...
            if cache[eid]['status'] not in ['ok','no_grounding'] or baseline['score_logits']!=cache[eid]['score_logits']['baseline']:
                raise ValueError('Actual baseline does not exactly reproduce frozen F7 finite-step baseline')
            ...
            logits=dict(baseline=baseline['score_logits'],F7=cache[eid]['score_logits']['all_query'],F8=candidate['score_logits'])
            row.update(status='no_grounding' if no_roi else 'ok',score_logits=logits,
                ...
                baseline_exact_to_f7_diagnostic=True,actual_baseline_measured=True,actual_noROI_fallback_applied=no_roi,
    ...
    create_json(out/'completion_v1.json',dict(time=timestamp(),summary=aggregate(rows,ids),errors=errors,
        records_sha256=digest(out/'records_v1.jsonl'),ordinal_objective_first_order_group_slopes=slopes,
        all1942_actual_baseline_measured=all(r.get('actual_baseline_measured') for r in rows),
        interpretation='pre-frozen fit-only comparison of F8 and F7 at same b1/k48; native readout unchanged; no efficacy/generalization gate',
