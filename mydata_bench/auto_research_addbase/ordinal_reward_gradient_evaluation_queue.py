"""Durable F8 evaluation queue. Stops on prerequisite failure; never retries."""
...
def run_stage(gpu, case, module, arguments, events, env):
    ...
    with (DEST/f'f8_evaluation_{case}_{module}_{stage}_stdout_v1.log').open('x') as handle:
...
def worker(gpu, cases, development):
    events = DEST/f'f8_evaluation_gpu{gpu}_events_v1.jsonl'
    ...
    for item in cases:
        ...
        for module, config, stage in stages:
            ...
            if code:
                ...
                break
    create_json(DEST/f'f8_evaluation_gpu{gpu}_completion_v1.json', dict(time=timestamp(), failures=failures,
...
def main():
    ...
    create_json(DEST/'f8_evaluation_queue_plan_v1.json', dict(time=timestamp(), pid=os.getpid(),
        source_sha256=digest(__file__), policy_sha256=digest(policy_path),
        dependencies_sha256=policy['evaluation_source_sha256'], gpus=[1, 2],
    ...
    while not done_path.exists():
        ...
        if not process.exists() or b'mydata_bench.auto_research_addbase.ordinal_reward_gradient_fit_queue' not in process.read_bytes():
            raise RuntimeError('F8 fitting queue ended without completion; inspect preserved evidence')
        time.sleep(20)
    if json.loads(done_path.read_text())['failures']:
        create_json(DEST/'f8_evaluation_queue_prerequisite_failure_v1.json', dict(time=timestamp(),
            cause='fitting queue reports failures', fitting_completion_sha256=digest(done_path)))
        raise RuntimeError('Required F8 fits failed; no evaluation launched')
    ...
    for module in ['freeze_ordinal_reward_gradient_development', 'freeze_ordinal_reward_gradient_validation']:
        ...
        if result.returncode:
            raise RuntimeError('F8 evaluation freeze failed: '+module)
    plan = json.loads((DEST/'ordinal_reward_gradient_validation_frozen_plan_v1.json').read_text())
    dev_plan = json.loads((DEST/'ordinal_reward_gradient_development_frozen_plan_v1.json').read_text())
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        jobs = [pool.submit(worker, 1, a, development), pool.submit(worker, 2, b, development)]
        failures = [row for job in jobs for row in job.result()]
    create_json(DEST/'f8_all_evaluation_queues_completion_v1.json', dict(time=timestamp(), failures=failures,
        reserved_test_not_evaluated=True, final_goal_completed=False))
    if failures:
        raise RuntimeError('F8 evaluation stage failures retained; inspect before any validation label join')
    ...
    if json.loads(f7_done.read_text())['failures']:
        raise RuntimeError('Fixed F7 comparator has stage failures; preserve before F8 label join')
    for module, arguments in [('analyze_ordinal_reward_gradient_development', []), ('analyze_ordinal_reward_gradient_validation', [])]:
        with (DEST/('f8_evaluation_'+module+'_stdout_v1.log')).open('x') as handle:
        ...
        if result.returncode:
            raise RuntimeError('Frozen F8 analysis failed: '+module)
    create_json(DEST/'f8_evaluation_and_analysis_completion_v1.json', dict(time=timestamp(),
