"""F7 all-query old-development inference: externally fitted signed heads, native score readout."""
import argparse
import fcntl
import json
import os
    ...
    if args.stage == 'engineering':
        ...
        for sample in samples:
            rr = {name: engineering.get((sample['example_id'], name), {}) for name in
                  ['baseline', 'original_all_k48', 'reward_gradient_k48', 'reward_gradient_zero_k48', 'learned_readout_k48']}
            checks.append(dict(example_id=sample['example_id'], complete=all(r.get('status') == 'ok' for r in rr.values()),
                ...
                target_all_queries=rr['reward_gradient_k48'].get('hook_diagnostics',{}).get('all_causal_queries',False),
                control_readout_only=rr['learned_readout_k48'].get('hook_diagnostics',{}).get('only_readout_query_changed',False)))
