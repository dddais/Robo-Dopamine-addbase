"""Differentiable per-head ROI bias at the actual reward readout query only."""
import copy
...
@contextmanager
def reward_gradient_steering(layers, head_biases, target, visual, readout_query_index,
    ...
    diag.update(calls=0, query_index=readout_query_index, active_heads=len(active_heads),
                only_readout_query_changed=True, target_count=len(set(target)), visual_count=len(set(visual)))
    ...
    def attend(module, q, k, v, mask, dropout=0., scaling=None, **kwargs):
        ...
        row = slice(readout_query_index, readout_query_index+1)
        z = q[:, hh, row, :].float() @ k[:, kv].float().transpose(-1, -2)
        z = z * (scaling if scaling is not None else dim**-.5)
        if mask is None:
            keep = torch.arange(klen, device=q.device) <= readout_query_index
    ...
    name = 'addbase_reward_gradient_readout_v1'
...
def native_grade_loss(logits, grade, model):
    if grade not in [1, 2, 3, 4, 5]:
        raise ValueError('Native1–5 supervised training grade required')
    if model in ['qwen', 'rr']:
        if logits.shape != (5,):
            raise ValueError('All five native choice logits required')
        return -torch.log_softmax(logits.float(), -1)[grade-1]
    if model != 'meter' or logits.shape != (10,):
        raise ValueError('All ten trained Meter bins required')
    position = (grade-1)*9/4
    lower, upper = int(position), min(int(position)+1, 9)
    fraction = position-lower
    logp = torch.log_softmax(logits.float(), -1)
    return -(1-fraction)*logp[lower]-fraction*logp[upper]
