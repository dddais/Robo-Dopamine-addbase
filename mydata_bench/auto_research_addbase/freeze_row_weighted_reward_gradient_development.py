"""Freeze F9 all-query learned heads, controls and old-development inference after all fits."""
import json
import pathlib
import yaml
from .common import ROOT,create_json,timestamp
from .robust_prepare import DEST,digest
from .f9_fit_provenance import verify_fit_artifacts
...
def main():
    ...
    for path,sha in policy['evaluation_source_sha256'].items():
        if digest(ROOT/path)!=sha:
            raise ValueError('Prospective F9 evaluation source changed: '+path)
    queue_done=json.loads((DEST/'f9_fit_completion_v1.json').read_text())
    if queue_done['failures']:
        raise ValueError('All15 F9 fitting cases must succeed before any evaluation freeze')
    if digest(policy['fitting_plan_file']) != policy['fitting_plan_sha256']:
        raise ValueError('Frozen F9 fitting plan changed')
    fit_plan=json.loads(pathlib.Path(policy['fitting_plan_file']).read_text())
    fits={item['model']+'_'+item['protocol']:item for item in fit_plan['configurations']}
    plan=json.loads((DEST/'frame_transport_frozen_plan_v1.json').read_text())
    if len(fits)!=15 or set(fits)!={r['model']+'_'+r['protocol'] for r in plan['configurations']}:
        raise ValueError('F9 fit/original protocol scope mismatch')
    for name,item in fits.items():
        if digest(item['path']) != item['sha256']:
            raise ValueError('Frozen F9 fitting configuration changed')
        done_path=DEST/'row_weighted_reward_gradient_v1'/name/'fit/completion_v1.json'
        done=json.loads(done_path.read_text())
        if done['examples']!=1942 or done['fitting_episode_groups']!=446:
            raise ValueError('Full fitting cohort required')
        fit_cfg=yaml.safe_load(pathlib.Path(item['path']).read_text())
        audit_path=DEST/'f8_complete_fit_mechanism_audit_v1.json'
        if digest(audit_path)!=fit_plan['F8_complete_fit_audit_sha256']:
            raise ValueError('Frozen F8 source-gradient audit changed')
        audit=json.loads(audit_path.read_text())
        verify_fit_artifacts(done_path, fit_cfg, audit['cases'][name]['f8_records_sha256'], item['sha256'])
    configs=[]
    for item in plan['configurations']:
        name=item['model']+'_'+item['protocol']
        source=DEST/'row_weighted_reward_gradient_v1'/name/'fit'
        ...
        if digest(item['path']) != item['sha256']:
            raise ValueError('Frozen original-data configuration changed')
        ...
        cfg.update(clean_source_dir=cfg['output_dir'],output_dir=str(DEST/f'row_weighted_reward_gradient_development_v1/{name}'),
            ...
            primary_k=48,k_neighborhood=[32,48,64],stage='F9_all_query_externally_fitted_heads_old234_development',
            fitting_loss='native cumulative ordinal log loss',
            fitting_estimand='full row mean; episode-cluster sandwich variability',
            output_score_adjustment='none',ranking_supervised_source='separate official train fitting partition only')
        for filename in ['reward_gradient_attention.py','all_query_reward_gradient_attention.py','all_query_reward_gradient_runtime.py','all_query_reward_gradient_development.py','freeze_row_weighted_reward_gradient_development.py']:
        ...
        path=ROOT/f'mydata_bench/configs/v2_crossmodel/addbase_robust_row_weighted_reward_gradient_development_v1_{name}.yaml'
    ...
    create_json(DEST/'row_weighted_reward_gradient_development_frozen_plan_v1.json',dict(time=timestamp(),configurations=configs,
        ...
        training_validation_selection_still_required=True,validation_reused_for_model_selection_not_new_confirmation=True))
