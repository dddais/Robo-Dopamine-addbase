"""Freeze all final reserved-test F7 settings after full original-scope success."""
import json
import pathlib
import yaml
...
def main():
    old_root = DEST/'all_query_reward_gradient_full_old_v1'
    ...
    create_json(claim_path, dict(time=timestamp(), family='F7_supervised_signed_all_query_reward_gradient',
    ...
    out = DEST/'all_query_reward_gradient_confirmation_v1'
    ...
    for name, cfg, best in selected:
        ...
        cfg.update(output_dir=str(out/name), input_file=str(inputs), input_sha256=digest(inputs),
            ...
            stage='F7_once_only_reserved2831_confirmation', all846_previously_exposed=True,
        ...
        path = ROOT/f'mydata_bench/configs/v2_crossmodel/addbase_robust_all_query_reward_gradient_confirmation_v1_{name}.yaml'
