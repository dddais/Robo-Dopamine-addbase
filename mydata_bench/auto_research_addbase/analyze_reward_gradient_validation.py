"""Join only training-validation labels, after all frozen F6 cases finish."""
import collections
import json
import pathlib
import yaml
...
def main():
    plan_path = DEST/'reward_gradient_validation_frozen_plan_v1.json'
    ...
    # This is the first label join. Neither inference nor the fitting runner
    # opens this file. Reserved external test labels never enter this analysis.
    ...
    for item, cfg, done, rows in loaded:
        ...
        for k in [32, 48, 64]:
            ...
            if k == 48:
                ...
                for control in ['raw_readout_k48', 'learned_positive_k48', 'reward_gradient_wrong_k48', 'reward_gradient_low_k48']:
        ...
        cases[name] = dict(model=cfg['model'], protocol=cfg['protocol'], validation_point_gate=point_gate,
            zero_exact=zero_exact, summaries=summaries, effects=effects, paired_group_uncertainty=uncertainty,
    ...
    old_paths = sorted(DEST.glob('f6_frame_transport_analysis_*.json'))
    if not old_paths:
        raise ValueError('Full old234 F6 descriptive analysis is required for joint case eligibility')
    ...
    for name, case in cases.items():
        raw = DEST/f'reward_gradient_development_v1/{name}/development/predictions.jsonl'
    ...
    joint_counts = {model: sum(c['old234_and_validation_joint_point_gate'] for c in cases.values() if c['model'] == model)
                    for model in ['qwen', 'rr', 'meter']}
    path = DEST/('f6_training_validation_analysis_'+timestamp().replace(':', '').replace('+', '_')+'.json')
    create_json(path, dict(time=timestamp(), cases=cases, passing_inputs_by_model=counts,
        ...
        validation_can_select_candidates_and_is_not_final_confirmation=True,
