"""Join only training-validation labels, after all frozen F8 cases finish."""
import collections
import json
import pathlib
import yaml
...
def main():
    plan_path = DEST/'ordinal_reward_gradient_validation_frozen_plan_v1.json'
    ...
    # F8 joins these reused model-selection labels only after all15 predictions.
    ...
    for item, cfg, done, rows in loaded:
        ...
        for field in ['model','protocol','input_sha256','example_ids_sha256']:
            if f6_cfg[field]!=cfg[field]:
                raise ValueError('F6/F8 selection data mismatch')
        ...
        f7_rows = f7_primary_rows(policy, name, 'validation', cfg, ids)
    ...
    old_paths = sorted(DEST.glob('f8_frame_transport_analysis_*.json'))
    if not old_paths:
        raise ValueError('Full old234 F8 descriptive analysis is required for joint case eligibility')
    ...
    for name, case in cases.items():
        raw = DEST/f'ordinal_reward_gradient_development_v1/{name}/development/predictions.jsonl'
    ...
    joint_counts = {model: sum(c['old234_and_validation_joint_point_gate'] for c in cases.values() if c['model'] == model)
                    for model in ['qwen', 'rr', 'meter']}
    path = DEST/('f8_training_validation_analysis_'+timestamp().replace(':', '').replace('+', '_')+'.json')
