"""Join only training-validation labels, after all frozen F9 cases finish."""
import collections
import json
import pathlib
import yaml
...
from .f9_reference_comparison import f8_primary_rows


def main():
    plan_path = DEST/'row_weighted_reward_gradient_validation_frozen_plan_v1.json'
    ...
    # F9 joins these reused model-selection labels only after all15 predictions.
    ...
    fixed_references = {item['model']+'_'+item['protocol']: (
        f7_primary_rows(policy, item['model']+'_'+item['protocol'], 'validation', cfg, ids),
        f8_primary_rows(policy, item['model']+'_'+item['protocol'], 'validation', cfg, ids))
        for item, cfg, _, _ in loaded}
    ...
    for item, cfg, done, rows in loaded:
        ...
        for field in ['model','protocol','input_sha256','example_ids_sha256']:
            if f6_cfg[field]!=cfg[field]:
                raise ValueError('F6/F9 selection data mismatch')
        ...
        f7_rows, f8_rows = fixed_references[name]
        effects['primary_F7_CE_fitted_all_query_heads'] = effect(obs['reward_gradient_k48'], observations(f7_rows, labels, samples, cfg['model']))
        effects['primary_F8_episode_weighted_ordinal_heads'] = effect(obs['reward_gradient_k48'], observations(f8_rows, labels, samples, cfg['model']))
    ...
    old_paths = sorted(DEST.glob('f9_frame_transport_analysis_*.json'))
    if not old_paths:
        raise ValueError('Full old234 F9 descriptive analysis is required for joint case eligibility')
    ...
    for name, case in cases.items():
        raw = DEST/f'row_weighted_reward_gradient_development_v1/{name}/development/predictions.jsonl'
    ...
    joint_counts = {model: sum(c['old234_and_validation_joint_point_gate'] for c in cases.values() if c['model'] == model)
                    for model in ['qwen', 'rr', 'meter']}
    path = DEST/('f9_training_validation_analysis_'+timestamp().replace(':', '').replace('+', '_')+'.json')
    create_json(path, dict(time=timestamp(), cases=cases, passing_inputs_by_model=counts,
        ...
        validation_can_select_candidates_and_is_not_final_confirmation=True,validation_previously_used_for_model_selection=True,
