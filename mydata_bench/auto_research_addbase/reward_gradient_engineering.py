"""Actual-model gradient plumbing test on old engineering examples, no labels."""
import argparse
import json
import os
import pathlib
...
from .reward_gradient_attention import reward_gradient_steering, native_grade_loss
...
def main():
    ...
    out = DEST/'reward_gradient_engineering_v1'/name
    out.mkdir(parents=True, exist_ok=True)
    create_json(out/'launch_v1.json', dict(time=timestamp(), pid=os.getpid(), gpu=os.environ.get('CUDA_VISIBLE_DEVICES'),
        source_sha256=digest(__file__), kernel_sha256=digest(ROOT/'mydata_bench/auto_research_addbase/reward_gradient_attention.py'),
    ...
    passed = len(checks)==len(ids) and all(c['zero_inference_exact'] and c['original_cached_logits_exact']
        and c['finite_gradient'] and c['nonzero_gradient_count']>0 and c['first8_gradients_exact_zero']
