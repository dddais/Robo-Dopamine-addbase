...
def eligible_cases(analysis):
    ...
    if len(analysis['cases']) != 15 or not analysis['all_cases_complete'] or not analysis['old234_and_validation_joint_gate'] or any(n < 3 for n in counts.values()):
        raise ValueError('F9 has not met joint old234 + separate-training-validation eligibility')
    return sorted(cases)
...
def main():
    ...
    validation_plan_path = DEST/'row_weighted_reward_gradient_validation_frozen_plan_v1.json'
    ...
    dest = DEST/'row_weighted_reward_gradient_full_old_v1'
    create_json(dest/'full846_ids_v1.json', [r['example_id'] for r in rows])
    source_files = ['all_query_reward_gradient_full_old.py', 'freeze_row_weighted_reward_gradient_full_old.py',
    ...
    for name in names:
        if any(digest(r['path']) != r['sha256'] for r in [vals[name], devs[name]]):
            raise ValueError('Earlier frozen F9 config changed')
        ...
        cfg.update(output_dir=str(dest/name), old234_output_dir=str(old_out),
            ...
            stage='F9_complete_original846_descriptive',
        ...
        path = ROOT/f'mydata_bench/configs/v2_crossmodel/addbase_robust_row_weighted_reward_gradient_full_old_v1_{name}.yaml'
    ...
    create_json(dest/'frozen_plan_v1.json', dict(time=timestamp(), configurations=configs, examples=846,
        ...
        selection='every jointly eligible case; no selection by full846 F9 or reserved test outcomes',
    ...
    print('Frozen full846 F9 follow-up:', len(configs), 'cases; all original846 retained', flush=True)
