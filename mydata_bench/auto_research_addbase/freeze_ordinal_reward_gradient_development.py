"""Freeze F8 all-query learned heads, controls and old-development inference after all fits."""
import json
import pathlib
import yaml
...
def main():
    ...
    for path,sha in policy['evaluation_source_sha256'].items():
        if digest(ROOT/path)!=sha:
            raise ValueError('Prospective F8 evaluation source changed: '+path)
    queue_done=json.loads((DEST/'f8_all_fit_queues_completion_v1.json').read_text())
    if queue_done['failures']:
        raise ValueError('All15 F8 fitting cases must succeed before any evaluation freeze')
    if digest(policy['fitting_plan_file']) != policy['fitting_plan_sha256']:
        raise ValueError('Frozen F8 fitting plan changed')
    ...
    if len(fits)!=15 or set(fits)!={r['model']+'_'+r['protocol'] for r in plan['configurations']}:
        raise ValueError('F8 fit/original protocol scope mismatch')
    for name,item in fits.items():
        if digest(item['path']) != item['sha256']:
            raise ValueError('Frozen F8 fitting configuration changed')
        done_path=DEST/'ordinal_reward_gradient_v1'/name/'fit/completion_v1.json'
        done=json.loads(done_path.read_text())
        if done['examples']!=1942 or done['fitting_episode_groups']!=446:
            raise ValueError('Full fitting cohort required')
        for filename,key in [('ranking_v1.json','ranking_sha256'),('gradient_records_v1.jsonl','gradients_sha256')]:
            if digest(done_path.parent/filename)!=done[key]:
                raise ValueError('Frozen complete F8 fit records changed')
    configs=[]
    for item in plan['configurations']:
        name=item['model']+'_'+item['protocol']
        source=DEST/'ordinal_reward_gradient_v1'/name/'fit'
        ...
        cfg.update(clean_source_dir=cfg['output_dir'],output_dir=str(DEST/f'ordinal_reward_gradient_development_v1/{name}'),
            ...
            primary_k=48,k_neighborhood=[32,48,64],stage='F8_all_query_externally_fitted_heads_old234_development',
        ...
        for filename in ['reward_gradient_attention.py','all_query_reward_gradient_attention.py','all_query_reward_gradient_runtime.py','all_query_reward_gradient_development.py','freeze_ordinal_reward_gradient_development.py']:
        ...
        path=ROOT/f'mydata_bench/configs/v2_crossmodel/addbase_robust_ordinal_reward_gradient_development_v1_{name}.yaml'
    ...
    create_json(DEST/'ordinal_reward_gradient_development_frozen_plan_v1.json',dict(time=timestamp(),configurations=configs,
