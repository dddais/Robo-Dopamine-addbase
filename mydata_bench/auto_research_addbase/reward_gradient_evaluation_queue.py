"""Durable F6 evaluation queue. Stops on prerequisite failure; never retries."""
...
def run_stage(gpu, case, module, arguments, events, env):
    ...
    with (DEST/f'f6_evaluation_{case}_{module}_{stage}_stdout_v1.log').open('x') as handle:
...
def worker(gpu, cases, development):
    events = DEST/f'f6_evaluation_gpu{gpu}_events_v1.jsonl'
    ...
    for item in cases:
        ...
        stages = [('reward_gradient_development', dev['path'], 'engineering'),
                  ('reward_gradient_validation', item['path'], 'engineering'),
                  ('reward_gradient_validation', item['path'], 'validation'),
                  ('reward_gradient_development', dev['path'], 'development')]
        for module, config, stage in stages:
            ...
            if code:
                ...
                break
    create_json(DEST/f'f6_evaluation_gpu{gpu}_completion_v1.json', dict(time=timestamp(), failures=failures,
...
def main():
    ...
    create_json(DEST/'f6_evaluation_queue_plan_v1.json', dict(time=timestamp(), pid=os.getpid(),
    ...
    launch = json.loads((DEST/'f6_fit_queue_launch_v1.json').read_text())
    done_path = DEST/'f6_all_fit_queues_completion_v1.json'
    while not done_path.exists():
        process = pathlib.Path('/proc')/str(launch['pid'])/'cmdline'
        if not process.exists() or b'mydata_bench.auto_research_addbase.reward_gradient_training_queue' not in process.read_bytes():
            raise RuntimeError('F6 fitting queue ended without completion; inspect preserved evidence')
        time.sleep(20)
    if json.loads(done_path.read_text())['failures']:
        create_json(DEST/'f6_evaluation_queue_prerequisite_failure_v1.json', dict(time=timestamp(),
            cause='fitting queue reports failures', fitting_completion_sha256=digest(done_path)))
        raise RuntimeError('Required F6 fits failed; no evaluation launched')
    ...
    for module in ['freeze_reward_gradient_development', 'freeze_reward_gradient_validation']:
        ...
        if result.returncode:
            raise RuntimeError('F6 evaluation freeze failed: '+module)
    plan = json.loads((DEST/'reward_gradient_validation_frozen_plan_v1.json').read_text())
    ...
    create_json(DEST/'f6_all_evaluation_queues_completion_v1.json', dict(time=timestamp(), failures=failures,
        reserved_test_not_evaluated=True, final_goal_completed=False))
    if failures:
        raise RuntimeError('F6 evaluation stage failures retained; inspect before any validation label join')
    for module, arguments in [('robust_analyze', ['--family', 'f6']), ('analyze_reward_gradient_validation', [])]:
        with (DEST/('f6_evaluation_'+module+'_stdout_v1.log')).open('x') as handle:
        ...
        if result.returncode:
            raise RuntimeError('Frozen F6 analysis failed: '+module)
    create_json(DEST/'f6_evaluation_and_analysis_completion_v1.json', dict(time=timestamp(),
