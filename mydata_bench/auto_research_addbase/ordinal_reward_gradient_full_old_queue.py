"""Conditionally schedule all846 after F8 joint old-development/validation pass."""
...
from .freeze_ordinal_reward_gradient_full_old import eligible_cases


def worker(gpu, configurations):
    events = DEST/f'f8_full_old_gpu{gpu}_events_v1.jsonl'
    ...
    for cfg in configurations:
        ...
        for stage in ['engineering', 'full846']:
            ...
            with (DEST/f'f8_full_old_{name}_{stage}_stdout_v1.log').open('x') as handle:
            ...
            if code:
                ...
                break
    create_json(DEST/f'f8_full_old_gpu{gpu}_completion_v1.json', dict(time=timestamp(), failures=failures,
        cases=[r['model']+'_'+r['protocol'] for r in configurations]))
    return failures


def main():
    files = ['all_query_reward_gradient_full_old.py', 'freeze_ordinal_reward_gradient_full_old.py',
             'analyze_ordinal_reward_gradient_full_old.py', 'ordinal_reward_gradient_full_old_queue.py']
    hashes = {p: digest(ROOT/'mydata_bench/auto_research_addbase'/p) for p in files}
    create_json(DEST/'f8_full_old_queue_plan_v1.json', dict(time=timestamp(), pid=os.getpid(), dependencies_sha256=hashes,
        prerequisite='all F8 evaluation/analysis complete and same >=3 inputs per model jointly pass old234+validation',
    ...
    launch = json.loads((DEST/'f8_evaluation_queue_launch_v1.json').read_text())
    done_path = DEST/'f8_evaluation_and_analysis_completion_v1.json'
    while not done_path.exists():
        ...
        if not process.exists() or b'mydata_bench.auto_research_addbase.ordinal_reward_gradient_evaluation_queue' not in process.read_bytes():
            create_json(DEST/'f8_full_old_queue_prerequisite_failure_v1.json', dict(time=timestamp(),
                cause='Evaluation queue stopped without completed validation analysis; no full846 inference launched'))
            raise RuntimeError('F8 evaluation prerequisite failed; inspect preserved evidence')
        time.sleep(20)
    ...
    candidates = sorted(DEST.glob('f8_training_validation_analysis_*.json'))
    if not candidates:
    ...
    try:
        ...
        create_json(DEST/'f8_full_old_queue_eligibility_v1.json', dict(time=timestamp(), eligible=False,
        ...
        return
    create_json(DEST/'f8_full_old_queue_eligibility_v1.json', dict(time=timestamp(), eligible=True,
        cases=names, analysis_file=str(analysis_path), analysis_sha256=digest(analysis_path), final_goal_completed=False))
    with (DEST/'f8_full_old_freeze_stdout_v1.log').open('x') as handle:
        result = subprocess.run([sys.executable, '-u', '-m', 'mydata_bench.auto_research_addbase.freeze_ordinal_reward_gradient_full_old',
    ...
    plan = json.loads((DEST/'ordinal_reward_gradient_full_old_v1/frozen_plan_v1.json').read_text())
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
        jobs = [pool.submit(worker, 1, a), pool.submit(worker, 2, b)]
        failures = [row for job in jobs for row in job.result()]
    create_json(DEST/'f8_full_old_inference_completion_v1.json', dict(time=timestamp(), failures=failures,
    ...
    with (DEST/'f8_full_old_analysis_stdout_v1.log').open('x') as handle:
        result = subprocess.run([sys.executable, '-u', '-m', 'mydata_bench.auto_research_addbase.analyze_ordinal_reward_gradient_full_old'],
    ...
    create_json(DEST/'f8_full_old_evaluation_and_analysis_completion_v1.json', dict(time=timestamp(),
