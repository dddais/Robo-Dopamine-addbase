"""Freeze all final reserved-test F8 settings after full original-scope success."""
import json
import pathlib
import yaml
...
def main():
    ...
    if priority['state']!='released':
        raise ValueError('F8 shared test not available: '+str(priority))
    old_root = DEST/'ordinal_reward_gradient_full_old_v1'
    ...
    create_json(claim_path, dict(time=timestamp(), F7_release_evidence=priority, family='F8_cumulative_ordinal_signed_all_query_reward_gradient',
    ...
    out = DEST/'ordinal_reward_gradient_confirmation_v1'
    ...
    for name, cfg, best in selected:
        ...
        cfg.update(output_dir=str(out/name), input_file=str(inputs), input_sha256=digest(inputs),
            ...
            stage='F8_once_only_reserved2831_confirmation', all846_previously_exposed=True,
        ...
        path = ROOT/f'mydata_bench/configs/v2_crossmodel/addbase_robust_ordinal_reward_gradient_confirmation_v1_{name}.yaml'
