...
import pytest
import yaml
from mydata_bench.auto_research_addbase.freeze_row_weighted_reward_gradient_confirmation import selected_confirmation_cases
from mydata_bench.auto_research_addbase import analyze_row_weighted_reward_gradient_confirmation as analyzer
...
def test_incomplete_reserved_outputs_block_test_label_join(tmp_path, monkeypatch):
    root = tmp_path/'row_weighted_reward_gradient_confirmation_v1'
    root.mkdir()
    ...
    claim.write_text(json.dumps(dict(family='F9_sample_mean_ordinal_signed_all_query_reward_gradient',policy_sha256=digest(dummy))))
...
def test_both_prior_families_must_be_scientifically_ineligible(tmp_path):
    from mydata_bench.auto_research_addbase.f9_confirmation_priority import confirmation_priority
    for family in ['f7','f8']:
        (tmp_path/(family+'_confirmation_queue_launch_v1.json')).write_text(json.dumps(dict(pid=999999999)))
        (tmp_path/(family+'_confirmation_queue_eligibility_v1.json')).write_text(json.dumps(dict(eligible=False,reserved_test_reward_inference_started=False)))
    ...
    assert confirmation_priority(tmp_path)['state']=='prerequisite_failed'
    (tmp_path/'f8_full_old_queue_eligibility_v1.json').write_text(json.dumps(dict(eligible=False)))
    assert confirmation_priority(tmp_path)['state']=='released'
    external=tmp_path/'external_roboreward_reserved_v1'
    external.mkdir()
    (external/'reward_model_confirmation_claim_v1.json').write_text(json.dumps(dict(family='F8')))
